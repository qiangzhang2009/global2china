...
// 支持的语言代码映射
export const SUPPORTED_LANGUAGES = ['zh', 'en', 'ja', 'ko', 'de', 'fr', 'es', 'it', 'pt', 'ru', 'ar'];
// 语言名称映射（用于显示）
...
export function getContactUsContent(lang = 'zh') {
    const isZh = lang === 'zh';
    const content = {
        zh: {
            title: '需要更专业的支持？',
...
export async function analyzeWithAI(productType, targetRegion, mode = 'sell-to-china', originRegion, options) {
...
function getDefaultAnalysis(productType, region, mode, options) {
    const productName = options?.productName || productType;
    const countryName = options?.countryName || '';
    const countryRegion = options?.countryRegion || '';
    // 中国区域消费特征
    const chinaRegionProfiles = {
        'east-china': {
            name: '华东地区',
...
export function getProductTypeFromCategory(level1Id, level2Id, level3Id) {
    const cat = PRODUCT_CATEGORIES.find((c) => c.id === level1Id);
    if (!cat)
        return 'food';
    const l2 = cat.children.find((c) => c.id === level2Id);
    if (!l2)
        return cat.children[0]?.id || 'food';
 ...
 */
export function getCategoryContextForAI(level1Id, level2Id) {
    const cat = PRODUCT_CATEGORIES.find((c) => c.id === level1Id);
    const l2 = cat?.children.find((c) => c.id === level2Id);
...
// 来源国按产品类型分组
export const ORIGIN_COUNTRIES_BY_PRODUCT = {
    supplement: [
...
// 获取指定产品类型对应的来源国列表
export const getOriginCountriesByProduct = (productId) => {
...
export const MARKETS_BY_REGION = {
    'east-asia': [
...
export const getMarketsByCategory = (regionId) => {
    return MARKETS_BY_REGION[regionId] || MARKETS_BY_REGION['east-asia'];
};
// 新接口：按区域获取市场列表
export const getMarketsByRegion = (regionId) => {
    return MARKETS_BY_REGION[regionId] || MARKETS_BY_REGION['east-asia'];
};
